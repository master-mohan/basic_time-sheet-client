...
function EmployeeTasks() {
  ...
  const verify = async (key, event, name) => {
    ...
    try {
      if (event.target.value !== "") {
        await axios.put(
          `http://localhost:5050/e1/employee/task/aprove/${key}`,
          {
            projectName: name,
      ...
      const updates = tasks.map((data) => {
        if (key === data._id) {
          data.approver = response.data[0].employeeName;
  ...
  const onSearch = async (e) => {
    ...
    if(src.length>=3){
      ...
      const filterData = async () => {
        ...
        const response = await axios.get(
          `${url.allTaskList}?search=${searchItem}`,
...
const handleFilterSubmit = async(event)=>{
  ...
  try {
    ...
    const response = await axios.get(
      `${url.allTaskList}?filter=${encodeURIComponent(filter)}`,
                            ...
                            <tr className="odd" key={data._id}>
                              ...
                              <td className="text-end">
                                <select
                                  ...
                                  id="verify567"
                                  onChange={(e) =>
                                    verify(data._id, e, data.projectName)
                                  }
                                >
                                  <option value="approved"  id="verify678">Approved</option>
                                  <option value="rejected"  id="verify678">Rejected</option>
                                </select>
                              </td>
                            </tr>
                          );
                        }) : <td colspan="10"><div class="d-flex text-center w-100 align-content-center justify-content-center">No matching records found</div></td>}
                      </tbody>
                    </table>
                  </div>
                  <div className="row">
